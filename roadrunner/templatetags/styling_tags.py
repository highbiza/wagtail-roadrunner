...
def get_styling(styling, no_style_tag=False):
    ...
    rule_set = [
        format_prop(prop, val) for prop, val in styling.items() if (
            val and not prop in style_classes
        )
    ]
...
def get_styling_classes(styling):
    if not styling:
        return
    klasses = [
        klass for title, klass in styling.items() if (
            klass and title in style_classes
        )
    ]
    return " ".join(klasses)
